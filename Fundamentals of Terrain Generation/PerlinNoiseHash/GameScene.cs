...
public class GameScene : Scene
{
    ...
    {
        _canvas = new Canvas (GraphicsDevice, "PerlinNoiseHash", Core.ScreenWidth / 2, Core.ScreenHeight / 2, 256, 256, 2);
        ...
        for (int x = 0; x < _canvas.TextureWidth; x++)
        {
            for (int y = 0; y < _canvas.TextureHeight; y++)
            {
                ...
                if (_domainWarpEnabled)
                {
                    ...
                    float warpedX = _perlinNoise.FractalBrownianMotionNoise (warpX + 211f, warpY + 163f, _seed, 4, 2f, 0.5f);
                    float warpedY = _perlinNoise.FractalBrownianMotionNoise (warpX + 127f, warpY + 107f, _seed, 4, 2f, 0.5f);
                ...
                float value = _fractalBrownianMotionEnabled
                     ? _perlinNoise.FractalBrownianMotionNoise (baseX, baseY, _seed, 4, 2f, 0.5f)
                     : _perlinNoise.Noise (baseX, baseY, _seed);
